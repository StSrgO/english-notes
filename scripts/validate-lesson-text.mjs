...
// D12 — стиль context.note (T22): пометка отвечает на вопрос «что это за материал и откуда он»,
// спокойным редакторским языком. Защитные оговорки о статусе материала и техножаргон — ошибки.
const NOTE_TECH = [
  [/учебн(?!ик)[а-яё]*/i, "«учебный пример/реконструкция/модель» — опиши материал, а не его статус"],
  [/не\s+(?:явля[а-яё]+|выдаются|выдаётся|выдаются)\s+(?:прямыми\s+|дословными\s+)?(?:цитат|словам)/i, "«не является цитатой» — оговорка о статусе"],
  [/не\s+выдаются\s+за|воспринима[а-яё]+\s+(?:его|её|их)?\s*как\s+(?:дословн|прям)/i, "«не выдаётся за / не следует воспринимать как…» — служебная оговорка"],
  [/приведен[а-яё]*\s+как\s+пример|для\s+демонстрации\s+грамматик/i, "«приведены как примеры / для демонстрации грамматики» — служебная пометка"],
  [/грамматическ[а-яё]+\s+(?:преобразовани|реконструкц)/i, "«грамматические преобразования/реконструкция» — техножаргон"],
  [/аналитическ[а-яё]+\s+(?:пример|конструкц|реконструкц|анализ)/i, "«аналитические примеры» — техножаргон"],
  [/собственн[а-яё]+\s+конструкц/i, "«собственные конструкции» — техножаргон"],
  [/аутентичн[а-яё]*/i, "«аутентичный» — техножаргон: скажи, откуда материал"],
  [/документированн[а-яё]*|подтвержда[а-яё]+/i, "«документированный/подтверждается» — техножаргон"],
];
...
for (const file of files) {
  ...
  for (const [p, s] of flat) {
    ...
    if (p === "context.note")
      for (const [re, note] of NOTE_TECH) {
        const m = s.match(re);
        if (m) err(file, p, m[0].trim(), `D12-стиль: ${note}`);
      }
  }
}
